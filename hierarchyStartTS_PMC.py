...
def ShowGraph(glists, ginfo, rowsize=3):
    ...
    plt.subplots(int(row), rowsize, figsize=(18, 10))
...
mmm=int(mmm)
print(f'故障节点上限是{mmm}')
print(f'随机分布{mmm}个故障节点')
xbh = []
xbh = random.sample(bh, mmm)
keys = bh
values = []
        ...
        for i in range(0, len(G1.adj[cur]), 1):  # 在Hsn中每个节点都有n个邻居节点
            ...
            if xx not in nodeSet:
                for j in range(0, len(G1.adj[xx]), 1):
                    yy = list(G1.adj[xx])[j]
                    if dictionary[cur] == dictionary[xx] : #PMC的比较方式
                        queue.append(xx)
...
G4 = nx.Graph()#数组U表示的是那些被孤立的结点，也是被误诊的结点
zcz = U
